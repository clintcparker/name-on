...
- [X] T022 Final 002 non-regression check per contract C8: `git diff main -- .github/workflows/verify-install-urls.yml install/install.sh install/install.ps1` is empty and `gh workflow run verify-install-urls.yml && gh run watch` is green (FR-008, SC-006)
